...
def get_top_most_parent(request):
    ...
    ) or Product.objects.filter(item_code=search_term).first()
    if not product:
    ...
    while current_product.parent_code:
        parent_product = Product.objects.filter(
            item_code=current_product.parent_code).first()
        if not parent_product:
            break
        current_product = parent_product

    child_products = list(Product.objects.filter(parent_code=product.item_code).order_by(
        'item_name').values_list('item_name', flat=True))
...
def average_product_price(request):
    l1_prices = Product.objects.values('category_l1').annotate(
        avg_price=models.Avg('mrp_price')).values_list('category_l1', 'avg_price')
    l2_prices = Product.objects.values('category_l1', 'category_l2').annotate(
        avg_price=models.Avg('mrp_price')).values_list('category_l1', 'category_l2', 'avg_price')
    print(l1_prices)
    print(l2_prices)
    context = {
        'l1_prices': l1_prices,
